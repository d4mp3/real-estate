...
function AddProperty() {
  ...
  function ReducerFunction(state, action) {
    switch (action.type) {
      ...
      case "CHANGE_MARKER_POSITION":
        return { ...state, latitudeValue: '', longitudeValue: '', markerPosition: { lat: action.changeLatitude, lng: action.changeLongitude } };
      default:
        return state;
    }
  }
  ...
  return (
    <div style={formContainer}>
      ...
      <Button
        onClick={() =>
          state.mapInstance && state.mapInstance.setView([51.53, -0.09], 15)
        }
